...
pprint (dm.get_contract_account_info("ETH"))

print (u' 获取用户持仓信息 ')
pprint (dm.get_contract_position_info())
pprint (dm.get_contract_position_info("ETH"))

print (u' 合约下单 ')
pprint(dm.send_contract_order(symbol='', contract_type='', contract_code='ETH181228', 
                        client_order_id='', price=50,volume=1,direction='buy',
                        offset='open',lever_rate=5,order_price_type='limit'))

# {'data': {'order_id': 42652052}, 'status': 'ok', 'ts': 1537188085856}

print (u' 合约批量下单??????? ')

print (u' 撤销订单 ')
pprint(dm.cancel_contract_order(order_id='42652052'))

print (u' 全部撤单 ')
pprint(dm.cancel_all_contract_order(symbol='ETH'))

print (u' 获取合约订单信息 ')
pprint(dm.get_contract_order_info(order_id='42652052'))

print (u' 获取合约订单明细信息 ')
pprint(dm.get_contract_order_detail(symbol='ETH', order_id='42652052'))
...
print (u' 获取合约历史委托 ')
pprint (dm.get_contract_history_orders(symbol='ETH', trade_type=0, type=1, status=0, create_date=7))
